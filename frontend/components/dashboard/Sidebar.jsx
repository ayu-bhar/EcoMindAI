...
// Accept props to control active state from parent
export default function Sidebar({ currentView, setView }) {
  const { logout, userData } = useAuth();

  const menuItems = [
    { id: 'dashboard', icon: LayoutDashboard, label: "Overview" },
    { id: 'personal_report', icon: FileText, label: "Personal Report" }, // Updated Name
    { id: 'resources', icon: Leaf, label: "Resources" },
    ...
    <aside className="w-64 backdrop-blur-2xl bg-black/20 border-r border-white/5 h-screen fixed left-0 top-0 flex flex-col z-50 shadow-2xl">
      ...
      <nav className="flex-1 px-4 space-y-1.5 mt-4">
        {menuItems.map((item) => (
          <button
            key={item.id}
            onClick={() => setView(item.id)} // Switch view on click
          ...
          >
            <item.icon size={18} className={`${currentView === item.id ? "text-green-400" : "text-zinc-500 group-hover:text-zinc-200"}`} />
            {item.label}
          </button>
        ))}
      </nav>
