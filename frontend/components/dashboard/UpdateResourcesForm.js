...
export default function UpdateResourcesForm({ onSubmit, initialData }) {
  const [step, setStep] = useState(1);
  const [formData, setFormData] = useState({
    energySources: initialData?.energySources || [],
    waterConsumption: initialData?.waterConsumption || '',
    wasteManagement: initialData?.wasteManagement || '',
    publicTransport: initialData?.publicTransport || '',
    renewableEnergyAccess: initialData?.renewableEnergyAccess || '',
    recyclingFacilities: initialData?.recyclingFacilities || '',
  ...
  const isStepValid = () => {
    switch (step) {
      case 1: return formData.energySources.length > 0 && formData.waterConsumption && formData.wasteManagement;
      case 2: return formData.publicTransport && formData.renewableEnergyAccess && formData.recyclingFacilities;
    ...
    <div className="max-w-3xl mx-auto">
      ...
      <div className="mb-10 px-2">
        <div className="flex justify-between items-end mb-3">
          <div>
            <span className="text-xs uppercase tracking-[0.2em] text-zinc-500 font-bold">Calibration Progress</span>
      ...
      <div className="relative group">
        <form 
          ...
          className="relative z-10 backdrop-blur-2xl bg-white/[0.01] border border-white/10 rounded-[2rem] p-8 md:p-10 shadow-2xl"
        >
          ...
          {step === 1 && (
            <div className="animate-in fade-in slide-in-from-right-4 duration-500">
              <div className="flex items-center gap-4 mb-8">
                ...
                <h3 className="text-2xl font-bold text-white">Resource Metrics</h3>
              </div>

              <div className="space-y-6">
                <div>
                  <label className={labelClasses}>Update Primary Energy Sources</label>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    ...
                    <select className={inputClasses + " appearance-none"} value={formData.waterConsumption} onChange={(e) => handleChange('waterConsumption', e.target.value)}>
                  ...
                  <div>
                    <label className={labelClasses}><Trash2 className="w-3 h-3 inline mr-1" /> Waste Management</label>
                    <select className={inputClasses + " appearance-none"} value={formData.wasteManagement} onChange={(e) => handleChange('wasteManagement', e.target.value)}>
                      <option value="" className="bg-zinc-900">Select...</option>
          ...
          {step === 3 && (
            <div className="animate-in fade-in slide-in-from-right-4 duration-500">
              <div className="flex items-center gap-4 mb-8">
                ...
                <h3 className="text-2xl font-bold text-white">Adjust Priorities</h3>
              </div>
