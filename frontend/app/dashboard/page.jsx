    ...
    <div className="flex min-h-screen bg-[#080e0d] text-zinc-300 font-sans selection:bg-green-500/30">
      ...
      <main className="ml-64 flex-1 p-8 relative">
        ...
        <div className="relative z-10">
          ...
          {/* 2. CONDITIONAL RENDERING: Check if a plan is selected */}
          {selectedPlan ? (
            <div className="animate-in slide-in-from-bottom-4 duration-500">
              {/* Back Button */}
              <button 
                onClick={() => setSelectedPlan(null)}
                className="flex items-center gap-2 text-zinc-500 hover:text-green-400 mb-8 transition-colors group"
              >
                <ArrowLeft size={18} className="group-hover:-translate-x-1 transition-transform" />
                <span className="text-sm font-bold uppercase tracking-widest">Back to Overview</span>
              </button>

              {/* Render the ActionPlanDisplay with the selected plan's data */}
              {/* Note: Ensure your data structure matches what ActionPlanDisplay expects */}
              <ActionPlanDisplay 
                plan={selectedPlan.content} 
                communityData={selectedPlan.communityData} 
              />
            </div>
          ) : (
            <>
                  ...
                  <table className="w-full text-left text-sm">
                    ...
                    <tbody className="divide-y divide-white/5">
                      {plans.map((plan) => (
                        <tr 
                          key={plan.id} 
                          // 3. CLICK HANDLER: Set the selected plan
                          onClick={() => setSelectedPlan(plan)}
                          className="hover:bg-white/[0.03] transition-colors group cursor-pointer"
                        >
                          <td className="px-8 py-6 font-semibold text-zinc-100 group-hover:text-green-400 transition-colors">
                            {plan.title}
                          </td>
                          <td className="px-8 py-6">
                            <div className="flex justify-center">
                              <span className={`px-4 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest border
                                ${plan.priority === 'High' ? 'bg-orange-500/10 border-orange-500/20 text-orange-500' : 
                          ...
                          <td className="px-8 py-6 text-zinc-500 font-mono text-xs italic">
                            {plan.date}
                          </td>
                        </tr>
                      ))}
